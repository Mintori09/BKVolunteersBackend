...
export const getTemplates = catchAsync(async (_req: Request, res: Response) => {
    const data = certificatesService.listTemplates()
...
export const createTemplate = catchAsync(async (req: Request, res: Response) => {
    const data = certificatesService.createTemplate({
...
export const updateTemplate = catchAsync(async (req: Request, res: Response) => {
    ...
    const data = certificatesService.updateTemplate(id, {
        name: req.body?.name,
        type: req.body?.type,
    ...
    if (!data) {
        throw new ApiError(HttpStatus.NOT_FOUND, 'Khong tim thay template chung nhan')
    }

    return ApiResponse.success(res, data, 'Cap nhat template thanh cong')
})

export const deactivateTemplate = catchAsync(async (req: Request, res: Response) => {
    const id = String(req.params.id ?? '')
    const data = certificatesService.deactivateTemplate(id)

    if (!data) {
        throw new ApiError(HttpStatus.NOT_FOUND, 'Khong tim thay template chung nhan')
    }

    return ApiResponse.success(res, undefined, 'Da chuyen template sang INACTIVE')
})

export const listCampaignCertificates = catchAsync(
    async (req: Request, res: Response) => {
        const campaignId = String(req.params.campaignId ?? '')
        const data = certificatesService.listCampaignCertificates(campaignId)
...
export const generateCampaignCertificates = catchAsync(
    async (req: Request, res: Response) => {
        const campaignId = String(req.params.campaignId ?? '')
        const data = certificatesService.generateCertificates({
            campaign_id: campaignId,
            template_id: String(req.body?.template_id ?? ''),
            module_id:
                typeof req.body?.module_id === 'string' && req.body.module_id.trim()
...
export const renderCertificate = catchAsync(async (req: Request, res: Response) => {
    const id = String(req.params.id ?? '')
    const data = certificatesService.renderCertificate(id)

    if (!data) {
        throw new ApiError(HttpStatus.NOT_FOUND, 'Khong tim thay chung nhan')
    }
...
export const getCertificateDownload = catchAsync(
    async (req: Request, res: Response) => {
        const id = String(req.params.id ?? '')
        const data = certificatesService.getCertificateDownload(id)
...
export const revokeCertificate = catchAsync(async (req: Request, res: Response) => {
    const id = String(req.params.id ?? '')
    const reason =
        typeof req.body?.revoke_reason === 'string'
            ...
            : typeof req.body?.reason === 'string'
              ...
              : undefined
    const data = certificatesService.revokeCertificate(
        id,
...
export const reissueCertificate = catchAsync(async (req: Request, res: Response) => {
    const id = String(req.params.id ?? '')
    const data = certificatesService.reissueCertificate(id)
...
export const getMyCertificates = catchAsync(async (_req: Request, res: Response) => {
    ...
    return ApiResponse.success(res, data, 'Lay danh sach chung nhan cua sinh vien thanh cong')
})

export const verifyCertificate = catchAsync(async (req: Request, res: Response) => {
    const code = String(req.params.code ?? '')
    const data = certificatesService.verifyCertificate(code)
